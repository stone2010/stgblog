...
class EmotionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        full_text = f"[CLS] {user_input} [SEP] {assistant_output}"
...
def finetune(model, train_loader, epochs=2, lr=1e-4, device='cuda'):
    model.to(device)
    ...
    for epoch in range(epochs):
        ...
        if avg_loss < best_loss:
            best_loss = avg_loss
            torch.save(model.state_dict(), 'model_emotion.pt')
            print('New best emotion model saved!')
...
if __name__ == "__main__":
    ...
    if os.path.exists('model_pretrain.pt'):
    ...
    dataset = EmotionDataset('../public/ai_data/emotion_dialogs.jsonl', tokenizer)
    train_loader = DataLoader(dataset, batch_size=32, shuffle=True, num_workers=4)
    
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    print(f'Using device: {device}')
    
    model = finetune(model, train_loader, epochs=2, lr=1e-4, device=device)